	...
	create() {
		...
		for (let i = 0; i < this.directions.length; i++) {
			...
			if (x >= 0 && x < matrix[0].length && y >= 0 && y < matrix.length && matrix[this.directions[i][1]][this.directions[i][0]] == 0) {
				...
				if (randomCh == 1 && number <= 3) {
					matrix[y][x] = 1;
					let newGr = new Grass(x, y, 1);
					grassArr.push(newGr);
					grassHashiv++;
				}
				else if (randomCh == 2 && number >= 3 && number < 6) {
					matrix[y][x] = 2;
					let newGrEater = new GrassEater(x, y, 2);
					grassEaterArr.push(newGrEater);
					grassEaterHashiv++;
				}
				else if (randomCh == 3 && number >= 6 && number <= 9) {
					matrix[y][x] = 3;
					let newPredator = new Predator(x, y, 3);
					predatorArr.push(newPredator);
					predatorHashiv++;
				}
				else if (randomCh == 6 && number > 9 && number <= 10) {
					matrix[y][x] = 3;
					let newHunter = new hunter(x, y, 6);
